...
#define LOGE(...)
#endif
#define LOGI2(...) __android_log_print(ANDROID_LOG_INFO  , TAG, __VA_ARGS__)
...
int read_pid(const char* pid_file){
	...
	if(fd < 0) {
		LOGE("read_pid -- open file [%s] fail,%d,%s", pid_file,errno, strerror(errno));
		return -1;
	}
	...
	while (fgets(path, sizeof(path)-1, fp) != NULL) {
		if(strstr(path,file_path)){
			...
			if(pid != opid){
				...
				if(res == 0){
					count++;
					LOGE("Kill Process Success!----- Pid=%d",opid);
				}else{
					LOGE("Kill Process Error!----- Pid=%d",opid);
				}
			}
		}
	}
...
bool singleton(const char *lock_file)
{
	...
	if(fd < 0){
		LOGE("singleton::open %s error,%d,%s",lock_file,errno,strerror(errno));
		return false;
	}

    rv = flock(fd, LOCK_EX|LOCK_NB);
    if (rv == -1) {
		LOGE("singleton::flock error,%d,%s",errno,strerror(errno));
        close(fd);
		return false;
    }   
...
bool set_running(const char* pid_file,int pid){
	...
	if(fd<0) {
		LOGE("set_running -- open file [%s] fail,%d,%s", pid_file,errno, strerror(errno));
		return -1;
	}
...
void *thread2(void *p)
{
	LOGI("thread2 : I'm thread 2");
	while(true)
	{
		if(access("/sdcard/1.exit",F_OK)!=-1){
			 LOGI("thread2 : exit process");
...
void *threadPushServiceTimer(void *p){
	...
	char tmp[1024] = { 0 };
	if(!strcmp(user,"null")){
		sprintf(tmp,"am startservice -n com.qihoo.around/com.qihoo.around.service.GuardService");
	}else{
		sprintf(tmp,"am startservice --user %s -n com.qihoo.around/com.qihoo.around.service.GuardService",user);
	}
	
	while(true){
		LOGI("threadPushServiceTimer is running...");
		sleep(10 * 60);
		system(tmp);
	}
...
{    
	...
	{    
		...
		{    
			size -= tmp;
			bytesOfRead += tmp;
			LOGD("size=%d,bytesOfRead=%d,tmp=%d", size,bytesOfRead,tmp); 			
		}     
...
void sigAlarm(int sig){
	...
	if(!strcmp(mUser,"null")){
		...
		sprintf(tmp,"am startservice --user %s -n com.qihoo.around/com.qihoo.around.service.GuardService",mUser);
	}
	LOGI("Alarm resume Guard Service...");
	system(tmp);
	alarm(10 * 60);
}
...
*/
    ...
    {
        int i=0;
		LOGI("child WATCHER ---- pid=%d,%d",getpid(),i++);
		while(true){
...
/*
	...
	*/
	if(!singleton(lock_file)){
		...
		LOGI("Already have an instance running");
		int pid = read_pid(pid_file);
		LOGI("kill proc %d",pid);
		kill(pid,9);
		...
		for(int i=0;i<10;i++){
			if(!singleton(lock_file)){
				...
				bSingleton = true;
				LOGI("singleton successed..............");
			}
		}
		if(!bSingleton){
			LOGE("singleton error..............");
			return 0;
			//exit(0);
		}
	...
	while(true)
	{
		...
		LOGI("select-------------------------retval=%d",retval);
		if(retval == 0){
			continue;
		
		}
		else if(retval == -1){
			LOGI("select error,%d,%s\n",errno,strerror(errno));
...
//			exit(0);
		...
		while ( i < length ) 
		{
			...
			if ( event->len ) 
			{
				LOGI("has event...%s,idx:%d,length:%d;event length=%d,event struct size=%d\n",event->name,i,length,event->len,EVENT_SIZE);
				usleep(10);
				int eidx = -1;
				if(eidx < 0){
					
						LOGI("uninstalled***************---%s",event->name);
